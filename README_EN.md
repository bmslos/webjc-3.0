# Web Vulnerability Scanner Pro v3.1.2
...
| **High-Performance Architecture** | Async orchestration + thread-pool concurrent scanning with semaphore control |
| **Intelligent Crawler** | JavaScript rendering, API endpoint discovery, automatic form recognition |
| **False Positive Control** | Two-layer deduplication (Exact/Input Point) + Secondary Verification + AI Filtering |
...
3. **Deduplicate**: Two-layer cross-deduplication eliminates duplicate reports
...
**Security Fixes**
- Rewrote CSRF API endpoint detection logic (previous response-header check was ineffective)
- CSV report formula injection protection

**Bug Fixes**
- `--resume`/batch scanning created duplicate tasks, causing task record bloat
- SQL boolean-based blind injection URL parameter concatenation error (this detection was effectively broken)
- Real vulnerabilities deleted when all verification requests failed (now kept as inconclusive)
- `--quiet` silent mode was reset by subsequent Logger calls
- Deduplication engine incorrectly merged parameterless vulnerabilities (security headers/CORS/sensitive files, etc.)
- File upload detection now performs real multipart uploads
- XSS reflection false positives (pages with their own script/svg tags no longer trigger)
- Sensitive file soft-404 false positives

**Architecture Optimization**
- Removed ~500 lines of dead code: unused async HTTP chain (AsyncHTTPUtils), dead L3 dedup layer, unconnected smart payload generation, sample plugin (duplicate XXE execution)
- Cleaned up unreferenced config entries and dependencies; fixed Playwright resource management and request interception timing

**Quality Improvements**
- Tests increased from 31 to 44, covering all key fixes
- flake8 F/E9 checks pass with zero warnings
