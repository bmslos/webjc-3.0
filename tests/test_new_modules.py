...
def test_dedup_engine():
    ...
    # page?id=1 的3种漏洞合并为1条（保留最严重），page?name=test 保留1条
    assert stats['final_output'] == 2, f"输出数量错误: {stats['final_output']}"

    # 合并后的漏洞应保留关联类型信息
    merged = [v for v in result if v['parameter'] == 'id'][0]
    assert 'XSS(反射型)' in merged.get('related_types', []), "关联类型信息丢失"
...
def test_dedup_parameterless_vulns_not_merged():
    """测试无参数漏洞（安全头/CORS/敏感文件等）不被L2误合并"""
    from core.dedup_engine import DedupEngine

    engine = DedupEngine()

    vulns = [
        {'url': 'http://example.com/', 'parameter': '', 'type': '安全头缺失', 'severity': '中危'},
        {'url': 'http://example.com/', 'parameter': '', 'type': 'CORS错误配置', 'severity': '高危'},
        {'url': 'http://example.com/', 'parameter': '', 'type': '敏感文件泄露', 'severity': '低危'},
    ]

    result = engine.deduplicate(vulns)

    # 无参数漏洞各自保留，不因同一URL被合并丢失
    assert len(result) == 3, f"无参数漏洞被误合并: {len(result)}"
    types = {v['type'] for v in result}
    assert types == {'安全头缺失', 'CORS错误配置', '敏感文件泄露'}

    print("[PASS] 无参数漏洞不被误合并")
...
if __name__ == '__main__':
    ...
    test_dedup_engine()
    test_dedup_parameterless_vulns_not_merged()
