...
| **高性能架构** | 异步调度 + 线程池并发扫描，支持信号量并发控制 |
| **智能爬虫** | JavaScript 渲染、API 端点发现、表单自动识别 |
| **误报控制** | 两层去重（精确/输入点）+ 二次验证 + AI 过滤 |
...
**安全修复**
- CSRF API 端点检测逻辑重写（原检查响应头无效）
- CSV 报告公式注入防护

**Bug 修复**
- `--resume`/批量扫描重复创建任务导致任务记录膨胀
- SQL 布尔盲注 URL 参数拼接错误（该检测此前实际失效）
- 验证请求全部失败时误删真实漏洞（改为保留待定状态）
- `--quiet` 静默模式被后续 Logger 调用重置失效
- 去重引擎误合并无参数漏洞（安全头/CORS/敏感文件等）
- 文件上传检测改为真实 multipart 上传
- XSS 反射判断误报（页面自带 script/svg 标签不再触发）
- 敏感文件软 404 误报

**架构优化**
- 移除未使用的异步 HTTP 链路（AsyncHTTPUtils）、L3 死去重层、未接入的智能 payload 生成、示例插件（XXE 双重执行）等约 500 行死代码
- 清理零引用配置项与依赖，Playwright 资源管理与请求拦截时机修复

**质量提升**
- 测试从 31 个增至 44 个，覆盖全部关键修复项
- flake8 F/E9 检查零告警
