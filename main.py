...
def run_single_scan(args, target_url: str, task_manager=None, task_id: Optional[str] = None):
    """
    执行单个目标的扫描
...
def main():
    ...
    if args.resume:
        ...
        for task in interrupted:
            ...
            run_single_scan(args, task['target_url'], task_manager, task_id=task['task_id'])
        return
    ...
    if not args.quiet and args.target:
        ...
        logger.info(f"扫描模式: {'同步' if args.sync else '异步(推荐)'}")
    ...
    # 多目标批量扫描
        ...
        # 按本次创建的任务逐个扫描，避免历史遗留的 pending 任务挤掉本次目标
        for idx, task_id in enumerate(task_ids, 1):
            task = task_manager.get_task(task_id)
            if not task:
                continue
            logger.info(f"\n{'=' * 70}")
            logger.info(f"扫描目标 [{idx}/{len(task_ids)}]: {task['target_url']}")
            logger.info(f"{'=' * 70}")
            run_single_scan(args, task['target_url'], task_manager, task_id=task_id)
