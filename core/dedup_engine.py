...
核心功能:
1. 基于漏洞指纹的精确去重（URL+参数+漏洞子类型的完整匹配）
2. 基于输入点的交叉去重（同一URL+同一参数的不同漏洞类型合并，仅限有参数漏洞）
3. 去重统计和报告
"""
...
class DedupEngine:
    ...
    def deduplicate(self, vulnerabilities: List[Dict]) -> List[Dict]:
        ...
        self.stats['final_output'] = len(l2_result)
        self.logger.info(
    ...
    def _l2_input_point_dedup(self, vulnerabilities: List[Dict]) -> List[Dict]:
        ...
        注意：无参数漏洞（安全头/CORS/敏感文件等）不参与合并，
        否则同一URL上的不同类型漏洞会被误合并丢失。
