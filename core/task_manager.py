...
class TaskManager:
    ...
    def create_batch_tasks(self, target_urls: List[str],
        ...
        config_json = json.dumps(config or {}, ensure_ascii=False)
        created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        task_ids = [f"task_{uuid.uuid4().hex[:16]}" for _ in valid_urls]
        ...
        with self._lock:
            ...
            cursor.executemany('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', [
                (task_id, url, self.TASK_STATUS_PENDING, priority, config_json, created_at)
                for task_id, url in zip(task_ids, valid_urls)
            ])
            conn.commit()
            conn.close()
