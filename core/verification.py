...
class VerificationEngine:
    ...
    def _verify_sqli_error(self, url: str, param: str,
        ...
        # 所有验证请求均失败（网络异常等），无法判定，不能当作误报删除
        if failed_count == len(error_payloads[:2]) and failed_count > 0:
            return {
                'status': 'inconclusive',
                'confidence': 0.5,
                'detail': '验证请求全部失败，无法确认或否定漏洞，保留原判定',
            }
    ...
    def _verify_sqli_time(self, url: str, param: str,
        ...
        # 所有验证请求均失败（网络异常等），无法判定，不能当作误报删除
        if failed_count == len(time_payloads[:2]) and failed_count > 0:
            return {
                'status': 'inconclusive',
                'confidence': 0.5,
                'detail': '验证请求全部失败，无法确认或否定漏洞，保留原判定',
    ...
    def _verify_xss(self, vuln: Dict) -> Dict:
        ...
        for payload in xss_payloads[:3]:
            ...
            if response and response.status_code == 200:
                # 仅匹配脚本执行特征，不匹配<script>等页面自身常见标签，避免误确认
                xss_markers = ['alert(', 'onerror=', 'onload=', 'ontoggle=', 'javascript:']
