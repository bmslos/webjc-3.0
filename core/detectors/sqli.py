...
class SQLInjectionDetector(BaseDetector):
    ...
    def _set_param(self, url: str, param: str, value: str) -> str:
        """替换URL中指定参数的值，保留其他参数并对value做URL编码"""
        parsed = urlparse(url)
    ...
    def _test_boolean_based(self, url: str) -> List[Dict]:
        ...
        for param in params[:2]:  # 只测试前2个参数
            for true_payload, false_payload in self.boolean_payloads[:2]:
                # 发送正常请求
                normal_url = self._set_param(url, param, '1')
                ...
                # 发送TRUE条件请求
                true_url = self._set_param(url, param, true_payload)
                ...
                # 发送FALSE条件请求
                false_url = self._set_param(url, param, false_payload)
    ...
    def _test_time_based(self, url: str) -> List[Dict]:
        ...
        for param in params[:2]:
            for sleep_payload in self.time_payloads[:3]:
                # 仅使用携带5秒延迟的payload，其余跳过
                if 'SLEEP(5)' not in sleep_payload and '00:00:05' not in sleep_payload and 'pg_sleep(5)' not in sleep_payload:
                    continue
