...
class CSRFDetector(BaseDetector):
    ...
    def _test_api_csrf(self, endpoint: Dict) -> List[Dict]:
        ...
        黑盒检测：POST请求成功且响应设置了缺少SameSite保护的会话Cookie时，
        浏览器不会阻止跨站请求携带该Cookie，可能存在CSRF风险。
        """
        vulnerabilities = []
        ...
        # 无会话Cookie时无法通过黑盒方式判定CSRF风险，跳过
        set_cookie = response.headers.get('Set-Cookie', '')
        if not set_cookie:
            return vulnerabilities

        # Cookie已设置SameSite保护时跨站请求会被浏览器拦截，跳过
        ...
        vulnerabilities.append({
            ...
            'parameter': 'N/A',
            'method': method,
            'payload': '会话Cookie缺少SameSite属性',
            'description': f'API端点 {url} 的会话Cookie未设置SameSite属性,跨站请求可携带Cookie,可能存在CSRF风险',
            'recommendation': '为会话Cookie设置SameSite=Lax/Strict属性。为所有状态修改的API端点添加CSRF token验证。要求X-CSRF-Token或X-Requested-With头'
        })
