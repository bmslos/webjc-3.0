...
class FileUploadDetector(BaseDetector):
    ...
    def _test_basic_upload(self, form: Dict) -> List[Dict]:
        ...
        # 尝试上传恶意文件（真实multipart/form-data上传）
            ...
            for malicious_file in self.malicious_files[:2]:  # 限制测试文件数量
                ...
                try:
                    response = self.http.post(
                        form_action,
                        data={'upload': 'test'},
                        files={file_field: (filename, content.encode('utf-8'), mime_type)}
                    )
                    ...
                    if response and response.status_code == 200:
                        ...
                        if any(keyword in response_text for keyword in ['uploaded', 'success', '上传成功', '文件路径', 'file path']):
                            vulnerabilities.append({
                                ...
                                'description': f'上传端点 {form_action} 接受了恶意文件 {filename} 的上传,需要进一步验证文件类型限制',
    ...
    def _test_bypass_techniques(self, form: Dict) -> List[Dict]:
            ...
            for bypass_file in bypass_files:
                try:
                    response = self.http.post(
                        form_action,
                        data={'upload': 'test'},
                        files={'file': (bypass_file['name'], bypass_file['content'].encode('utf-8'), bypass_file['mime'])}
                    )
            ...
            for case_file in case_files:
                try:
                    response = self.http.post(
                        form_action,
                        data={'upload': 'test'},
                        files={'file': (case_file['name'], case_file['content'].encode('utf-8'), case_file['mime'])}
                    )
        ...
        # 测试MIME类型操纵（以image/jpeg的MIME上传PHP内容）
        if 'mime_manipulation' in self.bypass_techniques:
            try:
                response = self.http.post(
                    form_action,
                    data={'upload': 'test'},
                    files={'file': ('test.jpg', b'<?php echo "test"; ?>', 'image/jpeg')}
                )
