...
class EnhancedWebScanner:
    ...
    def _crawl_sync(self):
        ...
        async def _run():
            # crawl与close必须在同一事件循环中执行，
            # 否则Playwright等异步资源会因跨事件循环而无法关闭
            try:
                return await crawler.crawl()
            finally:
                await crawler.close()

        crawled_data = asyncio.run(_run())
        self.crawled_data = crawled_data
