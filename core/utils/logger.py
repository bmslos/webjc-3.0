...
class Logger:
    ...
    def __init__(self, *args, **kwargs):
        # 从 kwargs 提取 verbose/quiet，兼容位置参数和关键字参数
        ...
        # 设置日志级别（quiet 静默模式优先，仅保留 WARNING 及以上）
        if quiet:
        ...
        if self.logger.handlers:
            self._sync_handler_levels(level)
            return
    ...
    def _sync_handler_levels(self, level: int):
        """同步调整已注册的控制台 handler 级别（文件 handler 固定 DEBUG）"""
