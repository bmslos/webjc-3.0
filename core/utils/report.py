...
def _csv_safe(value) -> str:
    """CSV注入防护：以=、+、-、@、制表符、回车开头的单元格前置单引号，防止公式执行"""
    ...
    def generate_csv(self, results: Dict[str, Any]) -> str:
        ...
        with open(report_file, 'w', encoding='utf-8-sig', newline='') as f:
            ...
            for idx, vuln in enumerate(vulnerabilities, 1):
                writer.writerow({
                    '编号': idx,
                    '漏洞类型': _csv_safe(vuln.get('type', '')),
                    '严重程度': _csv_safe(vuln.get('severity', '')),
                    'URL': _csv_safe(vuln.get('url', '')),
                    '参数': _csv_safe(vuln.get('parameter', '')),
                    '请求方法': _csv_safe(vuln.get('method', '')),
                    'Payload': _csv_safe(vuln.get('payload', '')),
                    '描述': _csv_safe(vuln.get('description', '')),
                    '修复建议': _csv_safe(vuln.get('recommendation', ''))
                })
